...
import { _StoreOfEditor } from './Store_ofEditor';
...
export const ViewPlugin_ofMain = () => {
  return ViewPlugin.fromClass(
    class Plugin {
      async update(update: ViewUpdate) {
        ...
        if (
          !update.docChanged ||
          !!update.state.field(StateField)?.suggestedText
        ) {
          return;
        }

        // Define which fetch function based on the model name
        const fetchFn = _$Utils._$Store.useGet(useSettingsStore)?.modelName.includes(
          'code-'
        )
